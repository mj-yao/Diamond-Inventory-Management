...
{
    public partial class fmWorkingLine : Form
    {
        ...
        public event EventHandler<CertificateDto> AddCertificate;
        ...
        public CertificateDto certificateData = new CertificateDto();

        private DocLineDto _docLineDto;
        private CertificateDto _certificateDto;
        ...
        public fmWorkingLine(DocLineDto docLineDto, CertificateDto certificateDto)
        {
            InitializeComponent();
            _docLineDto = docLineDto ?? new DocLineDto();
            _certificateDto = certificateDto ?? new CertificateDto();
        ...
        //----------------------------------Not In Use------------------------------------
        private void OldInitializeWorkingLineTable0()
        {
            List<string> columnNames = new List<string>();
            if (GridColumns.WorkingLineColumnByDocType.ContainsKey(_docTypeID))
            {
                columnNames = GridColumns.WorkingLineColumnByDocType[_docTypeID];
            }
        ...
        public void OldApplyColumnVisibility(DataGridView dgv, int docType)
        {
            // Ensure the document type exists in the hidden columns dictionary
            if (GridColumns.WorkingLineHiddenColumnsByDocType.ContainsKey(docType))
            {
                List<string> columnsToHide = GridColumns.WorkingLineHiddenColumnsByDocType[docType];
        ...
        {
            fmLotFinder lotFinder = new fmLotFinder();
        ...
        /*
        List<WorkingLineData> GetWorkingLineDataFromGrid()
        {
            var list = new List<WorkingLineData>();
            foreach (DataGridViewRow row in yourDataGridView.Rows)
            {
                var item = new WorkingLineData
                {
                    LineID = Convert.ToInt32(row.Cells["LineID"].Value),
                    LotName = row.Cells["LotName"].Value?.ToString(),
                    Quantity = Convert.ToDecimal(row.Cells["Quantity"].Value),
                    Weight = Convert.ToDecimal(row.Cells["Weight"].Value),
                    // map other cells...
                };
                list.Add(item);
            }
        ...
        */
        ...
        {
            fmCertificate certificate = new fmCertificate(_docTypeID, ref certificateData);
            certificate.Show();
        }
