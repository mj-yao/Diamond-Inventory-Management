...
namespace DiamondTransaction
{
    public partial class WorkingLine : Form
    {
        ...
        public WorkingLine(fmDiamondTransaction parentForm)
        {
            ...
            accountManager = new AccountDataManager(connectionString);
            accountManager.InitialiseLotNameComboBox(cb_LotName);
            accountManager.InitialiseParcelComboBoxGrading("Shape", cb_Shape);
            accountManager.InitialiseParcelComboBoxGrading("Size", cb_Size);
            accountManager.InitialiseParcelComboBoxGrading("Color", cb_Color);
            accountManager.InitialiseParcelComboBoxGrading("Clarity", cb_Clarity);
        ...
        private void FetchLotData(string lotID)
        {
            ...
            DataTable sqlResult = GlobalClass.AccessAndGetSqlResult(connectionString, query, errorMessageTitle, new Dictionary<string, object>
            {
                { "@LotID", lotID }
            });
        ...
        private void ConfirmLotNameInput()
        {
            ...
            else if (accountManager.LotNameList.Contains(lotName, StringComparer.OrdinalIgnoreCase))
            {
                InitializeDgvWorkingLineWithDefault0();
        ...
        */
        ...
        private void AccessDiamondLotMaxItemOrLotIDFor(string ParcelOrStone)
        {
            ...
            {
                GlobalClass.ShowErrorMessage(e1, "Access MAXItemID in THXADiamondLot failure.");
